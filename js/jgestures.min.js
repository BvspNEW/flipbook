...
(function (c) {
  ...
  c.jGestures.defaults.thresholdShake = {
    requiredShakes: 10,
    freezeShakes: 100,
    frontback: { sensitivity: 10 },
    leftright: { sensitivity: 10 },
    updown: { sensitivity: 10 },
  };
  ...
  c.jGestures.data = {};
  c.jGestures.data.capableDevicesInUserAgentString = [
    "iPad",
    "iPhone",
    "iPod",
    "Mobile Safari",
  ...
  c.jGestures.data.hasGestures = (function () {
    ...
    for (
      k = 0;
      k < c.jGestures.data.capableDevicesInUserAgentString.length;
      k++
    ) {
      if (
        navigator.userAgent.indexOf(
          c.jGestures.data.capableDevicesInUserAgentString[k]
        ) !== -1
      ) {
        return true;
      }
  ...
  function d(k) {
    k.startMove = k.startMove
      ? k.startMove
      : { startX: null, startY: null, timestamp: null };
    ...
    if (k.touches) {
      ...
      n[0].moved = Math.sqrt(
        Math.pow(Math.abs(n[0].startX), 2) + Math.pow(Math.abs(n[0].startY), 2)
      );
    }
    return {
      type: k.type || null,
      ...
      description:
        ...
        [
          ...
          k.touches,
          ":",
          n[0].lastX != 0 ? (n[0].lastX > 0 ? "right" : "left") : "steady",
          ":",
          n[0].lastY != 0 ? (n[0].lastY > 0 ? "down" : "up") : "steady",
        ].join(""),
    };
  }
  function a(l) {
  ...
  function b(r) {
    ...
    var n = o.oDeviceMotionLastDevicePosition || {
      ...
      shake: { eventCount: 0, intervalsPassed: 0, intervalsFreeze: 0 },
      shakeleftright: { eventCount: 0, intervalsPassed: 0, intervalsFreeze: 0 },
      shakefrontback: { eventCount: 0, intervalsPassed: 0, intervalsFreeze: 0 },
      shakeupdown: { eventCount: 0, intervalsPassed: 0, intervalsFreeze: 0 },
    };
    var q = {
    ...
    for (k in o) {
      switch (k) {
        ...
        case "shakeupdown":
          ...
          p.push(k);
          if (
            ++q[k].intervalsFreeze > m.freezeShakes &&
            q[k].intervalsFreeze < 2 * m.freezeShakes
          ) {
            break;
          }
          ...
          q[k].intervalsPassed++;
          if (
            (k === "shake" || k === "shakeleftright") &&
            (q.accelerationIncludingGravity.x > m.leftright.sensitivity ||
              q.accelerationIncludingGravity.x < -1 * m.leftright.sensitivity)
          ) {
            p.push("leftright");
            p.push("x-axis");
          }
          if (
            (k === "shake" || k === "shakefrontback") &&
            (q.accelerationIncludingGravity.y > m.frontback.sensitivity ||
              q.accelerationIncludingGravity.y < -1 * m.frontback.sensitivity)
          ) {
            p.push("frontback");
            p.push("y-axis");
          }
          if (
            (k === "shake" || k === "shakeupdown") &&
            (q.accelerationIncludingGravity.z + 9.81 > m.updown.sensitivity ||
              q.accelerationIncludingGravity.z + 9.81 <
                -1 * m.updown.sensitivity)
          ) {
            p.push("updown");
            p.push("z-axis");
          }
          if (p.length > 1) {
            if (
              ++q[k].eventCount == m.requiredShakes &&
              q[k].intervalsPassed < m.freezeShakes
            ) {
              t.triggerHandler(
                k,
                d({
  ...
  function j(r) {
    ...
    var y =
      Math.abs(m.oStartTouch.screenX - p) >
        c.jGestures.defaults.thresholdMove ||
      Math.abs(m.oStartTouch.screenY - o) > c.jGestures.defaults.thresholdMove
        ? true
        : false;
    var B =
      Math.abs(m.oStartTouch.screenX - p) >
        c.jGestures.defaults.thresholdSwipe ||
      Math.abs(m.oStartTouch.screenY - o) > c.jGestures.defaults.thresholdSwipe
        ? true
        : false;
    var A;
    var t;
  ...
  function e(l) {
    ...
    var n = {};
    n.oStartTouch = { timestamp: new Date().getTime() };
    ...
    for (n in q) {
      switch (n) {
        case "pinch":
          p = l.scale;
          if (
            (p < 1 && p % 1 < 1 - c.jGestures.defaults.thresholdPinchclose) ||
            (p > 1 && p % 1 > c.jGestures.defaults.thresholdPinchopen)
          ) {
            m = p < 1 ? -1 : +1;
            o = d({
              type: "pinch",
        ...
        case "rotate":
          p = l.rotation;
          if (
            (p < 1 && -1 * p > c.jGestures.defaults.thresholdRotateccw) ||
            (p > 1 && p > c.jGestures.defaults.thresholdRotatecw)
          ) {
            m = p < 1 ? -1 : +1;
            o = d({
              type: "rotate",
              ...
              description: [
                ...
                m,
                ":",
                p < 1 ? "counterclockwise" : "clockwise",
              ].join(""),
            });
            k.triggerHandler(o.type, o);
          }
